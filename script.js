async function generate() {
      ...
      if (!apiKey) return showError('Ingresá tu API key de Anthropic para continuar.');
  ...
  "cta": "call to action de no más de 8 palabras, con urgencia o beneficio claro"
      ...
      try {
        ...
        try {
          ...
          throw new Error('La IA respondió en un formato inesperado. Intentá de nuevo.');
        }

        // Render
        document.getElementById('titleContent').textContent = parsed.titulo || '';
        document.getElementById('descContent').textContent = parsed.descripcion_larga || '';
        document.getElementById('shortContent').textContent = parsed.descripcion_corta || '';
    ...
    function copyBlock(id, btn) {
      ...
      if (el.classList.contains('tags-list')) {
        text = Array.from(el.querySelectorAll('.tag-chip')).map(c => c.textContent).join(', ');
      } else {
        text = el.textContent;
      }
